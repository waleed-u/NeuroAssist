...
CREATE TABLE users (
    ...
    user_type ENUM('patient', 'doctor', 'staff') NOT NULL, -- Keep original types in DB
...
-- Doctors table (will be referred to as Consultants in the UI)
...
-- Staff table (will be referred to as Jr. Doctors in the UI)
CREATE TABLE staff (
    id INT PRIMARY KEY AUTO_INCREMENT,
    user_id INT NOT NULL,
    role VARCHAR(50),
...
CREATE TABLE eeg_analysis_details (
    id INT PRIMARY KEY AUTO_INCREMENT,
    eeg_record_id INT NOT NULL,  -- This should reference eeg_recordings.id
    ...
    FOREIGN KEY (analyzed_by) REFERENCES doctors(id)
);
...
FROM doctors d
LEFT JOIN patient_doctor_relationships pdr ON d.id = pdr.doctor_id
LEFT JOIN patients p ON pdr.patient_id = p.id
...
-- Insert test data for a doctor user (displayed as Consultant in UI)
INSERT INTO users (username, password_hash, email, full_name, user_type) VALUES
('dr.ahmed', '$2a$10$8K1p/a0dL1LXMIZoIqPK6.U/BOkNGx1k3hU9V3X3HJGQZsuHhJ6Hy', 'dr.ahmed@hospital.com', 'Dr. Ahmed Khan', 'doctor');
...
INSERT INTO patient_doctor_relationships (patient_id, doctor_id, start_date, status) VALUES
(1, 1, CURDATE(), 'active'),
(2, 1, CURDATE(), 'active'),
(3, 1, CURDATE(), 'active'),
(4, 1, CURDATE(), 'active'),
...
INSERT INTO eeg_records (patient_id, doctor_id, file_path, status) VALUES
