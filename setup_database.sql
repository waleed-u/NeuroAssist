...
CREATE TABLE users (
    ...
    user_type ENUM('patient', 'consultant', 'jr_doctor') NOT NULL,
...
-- Jr. Doctors table (renamed from Staff)
CREATE TABLE jr_doctors (
    id INT PRIMARY KEY AUTO_INCREMENT,
    user_id INT NOT NULL,
    role VARCHAR(50),
    department VARCHAR(100),
...
FROM consultants c
LEFT JOIN patient_consultant_relationships pcr ON c.id = pcr.consultant_id
...
-- Insert corresponding patient records
...
-- Insert test EEG record
INSERT INTO eeg_records (patient_id, consultant_id, file_path, status) VALUES
(1, 1, 'test_eeg.edf', 'pending');

-- Insert test data for new tables
INSERT INTO patient_medical_records (patient_id, record_date, diagnosis, treatment_plan, medications, created_by) VALUES
(1, CURDATE(), 'Initial consultation', 'Regular monitoring', 'None', 1);

INSERT INTO patient_vitals (patient_id, record_date, blood_pressure, heart_rate, temperature, weight, height, recorded_by) VALUES
(1, CURDATE(), '120/80', 72, 37.0, 70.5, 175.0, 1);

INSERT INTO eeg_analysis_details (eeg_record_id, analysis_type, findings, recommendations, analyzed_by) VALUES
(1, 'Standard EEG Analysis', 'Normal brain activity patterns observed', 'Continue regular monitoring', 1);
...
(6, DATE_SUB(NOW(), INTERVAL 10 DAY), 'patient5_eeg1.edf', 450, 256, 19, 'Parkinson\'s assessment');
